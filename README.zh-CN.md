...
![本地看板：逐账号积分、点卡片选用账号、最近路由带倍率，以及按地区分栏的模型列表](docs/dashboard.png)
...
- token 自动刷新与每日保活
- **本地看板** `GET /`：逐账号积分、签到状态与最近路由（每条带当次倍率）；点卡片或其上的按钮即可启用／停用账号，扫码添加账号、删除账号都能在浏览器里完成；另有「模型与倍率」页，按国内／国际分栏列出全部模型与倍率，按需刷新
...
| 共用接口 | 说明 |
|------|------|
| `GET /` | 看板：点卡片选用账号；没点亮的不参与路由 |
| `GET /admin/dashboard` | 看板 JSON；启用 API key 后需鉴权 |
| `POST /admin/dashboard/accounts` | 启用/停用账号（`{"auth_file":"a.info","enabled":true}`） |
...
浏览器打开 `http://127.0.0.1:8787/` 即可查看凭证池。每张卡片显示账号昵称、积分余额、积分最早过期时间、签到结果与黏绑会话数，下方列出最近路由及其实际承接账号。页面本身无需鉴权即可打开，但只输出页面骨架：账号数据来自 `GET /admin/dashboard`，启用 API key 后该接口需要鉴权，未携带时看板为空。

**点卡片即可选用或停用该账号。** 被停用的账号在选取凭证、构造请求头时会被跳过，也不出现在 `/v1/models` 里——只由停用账号提供的模型会从模型表中消失。所有账号默认启用，仅停用的账号会被记录在 `auth/dashboard-selection.json`，因此选择在重启后保留，新添加的账号默认启用。`POST /admin/dashboard/accounts` 可在不打开浏览器的情况下切换同一状态。

每张卡片上还有明确的**启用／停用**按钮（效果与点卡片一致），以及**删除**按钮。删除会真正 `unlink` 掉 `auth/<name>.info` 凭据文件，不可恢复，因此会**连续二次确认**。

同一页面的**添加账号**按钮可直接发起扫码登录：选择站点（国际／国内）后打开授权链接，页面持续轮询，成功后自动入库并刷新卡片。它调用的是与 `converter.py login` 完全相同的 `/admin/oauth/start`、`/admin/oauth/poll` 端点。启用 `--api-key` 后，账号数据与上述操作均需携带该 key。

**模型与倍率**页按国内／国际两栏列出全部模型及其倍率，倍率从低到高排序，`x0.00` 显示为「免费」。倍率取自各账号已加载到内存的模型目录缓存，**该页不会请求上游**。用页面上的**刷新模型列表**按钮按需刷新，按钮旁显示上次刷新时间，不设自动轮询。只由停用账号提供的模型不会列出，与路由行为一致。

最近路由会显示每条请求当次适用的倍率。**单次请求精确消耗的积分不显示**：上游用量明细是按「日 × 模型」聚合的，且路由记录里是客户端别名模型名，没有可靠映射到上游实际执行的模型。
