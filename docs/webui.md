...
Back up control metadata before using the new model rules. When reverting to older code, restore the matching control-database snapshot, including its WAL/SHM state without mixing old and new files; older readers reject the new rule fields.
