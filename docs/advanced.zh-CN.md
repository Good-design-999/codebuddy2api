...
每次刷新 `/v3/config` 都按账号保留两个作用域：agent **选择器**子集（`data.agents[cli].models`，
也就是官方客户端展示的那批名字）和账号**根表**（`data.models`）。选路与 `GET /v1/models` 用
「选择器 ∪ 根表」，因为子集只说明客户端要展示什么，不说明后端能服务什么：实测 2026-09-14 国内
账号根表 30 项、选择器只剩 16 项，被裁掉的 14 个支持工具调用的名字（`hy4-preview`、
`deepseek-v3-2-volc`、`glm-4.6` 等）直接请求全部回 `200`。根表仍然按账号隔离，所以该账号发不出去
的名字——国际站后端对 `deepseek-v3-2-volc` 回 `400`——既不会对外报名，也不会借给别的账号。
不支持工具调用的模型（图像/视频档）在两个视图里都会被过滤掉。`auth/model-catalog.json` 里选择器
子集仍存为 `models`，根表另存为 `serves`；升级前写的缓存没有该字段，自动退回子集。
